DROP INDEX IF EXISTS q5;
DROP INDEX IF EXISTS q5_2;
--CREATE INDEX q5 ON cats.likes(video_id, user_id);
--CREATE INDEX q5_2 ON cats.friend(user_id, friend_id);
...
PREPARE Q5 (int) AS
    ...
    t4 AS (
    SELECT t3.video_id, t3.other_user, t3.liked, l.iliked, (t3.liked AND l.iliked) AS liked_by_both
    ...
    iwatched AS (
    SELECT u.video_id, u.user_id, CAST(COALESCE(w.watch_id,0) AS boolean) AS iwatched
    ...
    t9 AS (
    SELECT t8.video_id, t8.rank, l.iliked, w.iwatched
